...
const EMAIL_SUPPORT = 'soporte@transworld.cl'; // Correo de registro
...
// ... (FUNCIONES GENERADORAS DE HTML Y TEXTO - SE MANTIENEN IGUAL) ...
...
router.get('/', (req, res) => res.render('sistemas/index', { titulo: 'Sistemas', user: req.session.user }));
...
router.get('/tickets', async (req, res) => {
  ...
  } catch (err) { console.error(err); res.status(500).send('Error consultando tickets'); }
});
...
// ... (Ruta CREAR TICKET - MANTENER IGUAL, Opcional agregar BCC aquí también si quieres) ...
  ...
  try {
    ...
    if (process.env.ADMIN_NOTIFY_EMAIL) {
      ...
      sendMail({
        ...
        bcc: EMAIL_SUPPORT // <--- COPIA A SOPORTE
      }).catch(console.error);
    }
    res.redirect(`/sistemas/tickets/${nuevoId}`);
  } catch (err) { console.error(err); res.status(500).send('Error al procesar el ticket.'); }
});

// ... (Ruta Signature - MANTENER IGUAL) ...
router.get('/tickets/signature', async (req, res) => {
  ...
  } catch (err) { console.error(err); res.status(500).json({ error: 'Error firma' }); }
});

// ==========================================
// ADMIN: GESTIONAR (Con Copia a Soporte)
// ==========================================
router.post('/tickets/:id/actualizar', requireRole('admin'), async (req, res) => {
  ...
  else if (estado === 'Abierto') sql += `, fecha_resolucion = NULL, fecha_cierre = NULL`;
    ...
    if (tieneMensaje || tieneAdjuntos) {
      ...
      if (ticket.length > 0) {
        ...
        sendMail({
          ...
          bcc: EMAIL_SUPPORT // <--- COPIA OCULTA A SOPORTE
        }).catch(console.error);
      }
    }
    res.redirect(`/sistemas/tickets/${id}`);
  } catch (err) { console.error(err); res.status(500).send('Error actualizando ticket'); }
});

// ... (Rutas confirmar/rechazar - MANTENER IGUAL) ...
router.post('/tickets/:id/confirmar', async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0 || rows[0].solicitante_email !== user.email) return res.status(403).send('No tienes permiso.');
    await db.query(`UPDATE tickets SET estado = 'Cerrado', fecha_cierre = NOW(), cierre_automatico = 0 WHERE id = ?`, [id]);
    res.redirect(`/sistemas/tickets/${id}`);
  } catch (err) { console.error(err); res.status(500).send('Error al confirmar solución'); }
});
...
router.post('/tickets/:id/rechazar', async (req, res) => {
  ...
  try {
    ...
    await db.query(`INSERT INTO ticket_respuestas (ticket_id, mensaje, remitente) VALUES (?, ?, ?)`, [id, 'El usuario ha rechazado la solución y el ticket se ha reabierto.', 'Sistema']);
    ...
    res.redirect(`/sistemas/tickets/${id}`);
  } catch (err) { console.error(err); res.status(500).send('Error al rechazar solución'); }
});

// ==========================================
// USUARIO: RESPONDER (Con Copia a Soporte)
// ==========================================
router.post('/tickets/:id/responder', async (req, res) => {
  ...
  try {
    ...
    res.redirect(`/sistemas/tickets/${id}`);
  } catch (err) { console.error(err); res.status(500).send('Error procesando respuesta.'); }
});

// ... (Ruta Detalle - MANTENER IGUAL) ...
router.get('/tickets/:id', async (req, res) => {
  const { id } = req.params;
  const user = req.session.user;
  const sqlTicket = `SELECT * FROM tickets WHERE id = ?`;
  const sqlRespuestas = `SELECT id, mensaje, remitente, fecha, archivo_url, archivo_nombre, archivo_tipo, adjuntos FROM ticket_respuestas WHERE ticket_id = ? ORDER BY fecha ASC`;
  ...
  try {
    ...
    if (user.role !== 'admin' && ticketResults[0].solicitante_email !== user.email) return res.status(403).send('No tienes permisos.');
    const [respuestasResults] = await db.query(sqlRespuestas, [id]);
    res.render('sistemas/tickets_detalle', { titulo: `Ticket #${id}`, ticket: ticketResults[0], respuestas: respuestasResults, user: user });
